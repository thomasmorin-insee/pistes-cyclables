-- Requête sur les données table_sql
...
SELECT
	CASE
		...
		-- Déplacé : AMENAGEMENT MIXTE PIETON VELO HORS VOIE VERTE
		...
		-- Remplacé : VOIE VERTE
		WHEN traffic_sign = 'FR:C115' OR ((bicycle = 'designated' OR designation = 'greenway') AND ((highway = 'path' AND (motor_vehicle IS NULL OR motor_vehicle = 'no' OR motorcar IS NULL OR motorcar = 'no')) OR (highway = 'track' AND tracktype = 'grade1'))) THEN 'VOIE VERTE'
	...
	CASE
		...
		-- Déplacé DOUBLE SENS CYCLABLE NON MATERIALISE
